...
def retrieve_args():
    ...
    args.device = torch.device(args.device) if args.device else torch.device(
...
def run_test(dataloader, model, criterion, device, compress):
    ...
    f1 = metrics.f1_score(labels_all.cpu(), torch.argmax(predicts_all, -1).cpu(), labels=[0, 1, 2],
...
def main():
    args = retrieve_args()
    args.tokenizer = build_tokenizer(fnames=args.dataset_file.values(),
                                     dataset=args.dataset)  # transfrom tokens to indices
    print(args.tokenizer.max_slen)
    print(args.tokenizer.max_tlen)
    print(args.tokenizer.max_plen)

    embedding_matrix = build_embedding_matrix(vocab=args.tokenizer.vocab['word'], dataset=args.dataset,
                                              dir_path='../..')  # pre-trained glove embeddings
    ...
    for epoch in range(args.num_epoch):
        ...
        if test_acc > best_test_acc:
            ...
            scheduler.step()
        print(
            f"{100 * (epoch + 1) / args.num_epoch:6.2f}% > loss: {train_loss:.4f}, acc: {train_acc:.4f}, test acc: {test_acc:.4f}, test f1: {test_f1:.4f}")
        print('', flush=True)
